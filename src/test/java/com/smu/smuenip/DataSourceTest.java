...
public class DataSourceTest {
    ...
    void masterDataSourceTest(@Qualifier("masterDataSource") final DataSource masterDataSource) {
        ...
        String driverClassName = environment.getProperty(
            "spring.datasource.master.hikari.driver-class-name");
        Boolean readOnly = Boolean.valueOf(
            environment.getProperty("spring.datasource.master.hikari.read-only"));
    ...
    void slaveDataSourceTest(@Qualifier("slaveDataSource") final DataSource slaveDataSource) {
        ...
        String driverClassName = environment.getProperty(
            "spring.datasource.slave.hikari.driver-class-name");
        Boolean readOnly = Boolean.valueOf(
            environment.getProperty("spring.datasource.slave.hikari.read-only"));
    ...
    private void verifyOf(Boolean readOnly, String url, String username, String driverClassName,
        HikariDataSource hikariDataSource) {
        Assertions.assertThat(hikariDataSource.isReadOnly()).isEqualTo(readOnly);
        Assertions.assertThat(hikariDataSource.getJdbcUrl()).isEqualTo(url);
        Assertions.assertThat(hikariDataSource.getUsername()).isEqualTo(username);
        Assertions.assertThat(hikariDataSource.getDriverClassName()).isEqualTo(driverClassName);
    }
}
